...
export default function CityScene({ onEnter, onMarket, rushUnlocked=false, cafeChanged=false }: { onEnter: () => void; onMarket?:()=>void; rushUnlocked?: boolean; cafeChanged?: boolean }) {
  ...
  return <div className="city-shell">
    ...
    <div className="city-world">
      ...
      {status !== 'ready' && <div className="city-fallback" role="status"><Coffee size={36}/><p>{status === 'loading' ? '首次在新设备打开时，沙盘加载时间可能较长，请稍等…' : '当前设备无法显示 3D 城市。'}</p>{status==='fallback'&&<><button onClick={onEnter}>进入咖啡店 <ArrowUpRight size={17}/></button>{onMarket&&<button onClick={onMarket}>走进同春市场 <ArrowUpRight size={17}/></button>}</>}</div>}
      <div className="city-compass"><span>北 N</span><i>↑</i></div>
      <div className="city-map-tools">
        <select aria-label="查找景点或场景" value={travel||selected||''} onChange={event=>selectPlace(event.target.value)}><option value="">寻找一个地点…</option>{CITY_PLACES.map(place=><option key={place.id} value={place.id}>{place.name}{place.id==='market'?' · 采购体验':place.status==='planned'?' · 即将开放':''}</option>)}</select>
        <div className="city-zoom"><button aria-label="重置地图视角" onClick={()=>changeView('overview')}><LocateFixed size={17}/></button></div>
      </div>
      <div className="city-legend"><span className="legend-dot"/> 咖啡任务 <span className="legend-muted"/> 城市地标 <small>拖动横移视角 · 点击地点前往</small></div>
