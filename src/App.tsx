...
type ChatMessage = { role: 'clerk' | 'user'; vi: string; zh?: string; source?: 'ai' | 'local' };
...
export default function App() {
  ...
  const greeting = (mode: Difficulty): ChatMessage => mode === 'rush' ? { role: 'clerk', vi: 'Chào bạn. Gọi món nhanh nhé, tôi đang rất bận.', zh: '你好。请快点单，我现在很忙。', source: 'local' } : { role: 'clerk', vi: 'Xin chào! Bạn muốn uống gì?', zh: '你好！你想喝什么？', source: 'local' };
...
function Chat({ messages, setMessages, assessment, setAssessment, hintsUsed, setHintsUsed, onComplete, difficulty }: { messages: ChatMessage[]; setMessages: Dispatch<SetStateAction<ChatMessage[]>>; assessment: Assessment; setAssessment: Dispatch<SetStateAction<Assessment>>; hintsUsed: number; setHintsUsed: Dispatch<SetStateAction<number>>; onComplete: (assessment: Assessment, messages: ChatMessage[], stats: RushStats) => void; difficulty: Difficulty }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const timeoutMessage: ChatMessage = { role: 'clerk', vi: `Bạn gọi món xong chưa? Nhanh lên giúp tôi, phía sau còn khách. ${next.vi}`, zh: `您点好了吗？请快一点，后面还有客人。${next.zh}`, source: 'local' };
