...
export default{
  async fetch(request,env){
    ...
    let upstream;
    try{upstream=await fetch('https://api.deepseek.com/chat/completions',{method:'POST',headers:{'Authorization':`Bearer ${env.DEEPSEEK_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:'deepseek-flash',messages:[{role:'system',content:prompt},...history],response_format:{type:'json_object'},max_tokens:180,temperature:.65,stream:false})})}catch{return json({error:'AI unavailable'},502,origin)}
