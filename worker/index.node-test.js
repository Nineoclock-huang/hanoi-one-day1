...
test('Worker returns a validated bilingual reply without exposing the key',async()=>{
  ...
  try{
    ...
    assert.deepEqual(await response.json(),{vi:'Bạn muốn loại cà phê nào?',zh:'你想要哪一种咖啡？',attempts:{product:'not_attempted',quantity:'correct',sugar:'not_attempted',service:'not_attempted',payment:'not_attempted'}});
...
test('Worker retries once when DeepSeek returns empty JSON content',async()=>{
  const originalFetch=globalThis.fetch;
  let calls=0;
  globalThis.fetch=async()=>{calls++;return new Response(JSON.stringify({choices:[{message:{tool_calls:calls===1?[]:[{function:{arguments:JSON.stringify({vi:'Bạn muốn dùng loại nào?',zh:'你想选择哪一种？',attempts:{}})}}]}}]}),{status:200,headers:{'Content-Type':'application/json'}})};
  try{
