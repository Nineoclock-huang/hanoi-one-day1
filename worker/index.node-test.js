...
test('Worker returns a validated bilingual reply without exposing the key',async()=>{
  ...
  let authorization='';
  globalThis.fetch=async(_url,options)=>{authorization=options.headers.Authorization;return new Response(JSON.stringify({choices:[{message:{content:JSON.stringify({vi:'Bạn muốn loại cà phê nào?',zh:'你想要哪一种咖啡？'})}}]}),{status:200,headers:{'Content-Type':'application/json'}})};
  try{
