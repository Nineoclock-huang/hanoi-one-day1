...
import { type AiFeedback, type ClerkMood, isAiConfigured, requestAiFeedback, requestAiReply } from './ai';
...
import { applyQuestionTimeout, averageResponseTime, formatCountdown, RUSH_QUESTION_LIMIT_MS, rushPenalty } from './rush';
...
type SavedReport = { score: number; objectiveScore: number; completedAt: string; expressions: string[]; criteria: Criteria; assessment: Assessment; hints: number; feedback: AiFeedback | null; feedbackUnavailable?: boolean; difficulty: Difficulty; timeouts: number; responseTimes: number[]; timePenalty: number };
...
export default function App() {
  ...
  const finish = async (finalAssessment: Assessment, finalMessages: ChatMessage[], stats: RushStats) => {
    ...
    const base: SavedReport = { score: objectiveScore, objectiveScore, completedAt: new Date().toISOString(), expressions: finalMessages.filter(message => message.role === 'user').map(message => message.vi), criteria: correctCriteria(finalAssessment), assessment: finalAssessment, hints: hintsUsed, feedback: null, difficulty, timeouts: stats.timeouts, responseTimes: stats.responseTimes, timePenalty };
    setScreen('grading');
    const feedback = await requestAiFeedback({ messages: finalMessages, target: currentOrderTarget, assessment: finalAssessment, difficulty, responseTimes: stats.responseTimes, timeouts: stats.timeouts });
    const result: SavedReport = feedback ? { ...base, score: Math.max(0, finalScore(finalAssessment, feedback.languageScore, hintsUsed) - timePenalty), feedback } : { ...base, feedbackUnavailable: true };
...
function Mission({ onStart, difficulty }: { onStart: () => void; difficulty: Difficulty }) { const rush = difficulty === 'rush'; return <div className={`page-wrap narrow ${rush ? 'rush-mission' : ''}`}><p className="eyebrow">{rush ? '限时挑战 · DẬN · A2' : '任务 01 · 街角咖啡店 · A1'}</p><h2>{rush ? '店员似乎发生了变化。' : '这次，请点'}<br />{rush ? 'Dận 正在等你点单' : orderSummary()}</h2><div className="scenario-note"><strong>{rush ? '新的挑战规则' : '你的情境'}</strong><p>{rush ? `每个问题只有 ${RUSH_QUESTION_LIMIT_MS / 1000} 秒。倒计时精确到毫秒，超时会锁定当前项目为错误并扣 5 分。` : '上午九点，你走进河内老城区的一家咖啡店。请用越南语完成这张随机订单。'}</p></div><div className="mission-card"><p>{rush ? 'Dận 的订单仍包含 5 个目标：' : '需要作答 5 个目标（每项只有一次评分机会）：'}</p><ul>{Object.values(criterionLabels).map((label, index) => <li key={label}><span>{String(index + 1).padStart(2, '0')}</span>{label}</li>)}</ul></div><p className="tip">{rush ? '思考要快，表达也要完整。AI 会保持店员身份并根据你的回答继续追问。' : '答错也能继续对话，但首次明确作答的结果会锁定。系统可以理解无声调、大小写和常见空格问题。'}</p><button className="primary-button" onClick={onStart}>{rush ? '接受限时挑战' : '进入咖啡店'} <ArrowRight size={20} /></button></div>; }
...
function Chat({ messages, setMessages, assessment, setAssessment, hintsUsed, setHintsUsed, onComplete, difficulty }: { messages: ChatMessage[]; setMessages: Dispatch<SetStateAction<ChatMessage[]>>; assessment: Assessment; setAssessment: Dispatch<SetStateAction<Assessment>>; hintsUsed: number; setHintsUsed: Dispatch<SetStateAction<number>>; onComplete: (assessment: Assessment, messages: ChatMessage[], stats: RushStats) => void; difficulty: Difficulty }) {
  ...
  useEffect(() => {
    if (!rush || !rushStarted || isSending || resolvedCount(timerState.current.assessment) === 5) return;
    ...
    const tick = () => {
      ...
      const state = timerState.current, timed = applyQuestionTimeout(state.assessment);
      if (!timed.key) return;
      const newTimeouts = state.timeouts + 1;
      const next = clerkReply(resolvedCriteria(timed.assessment), state.messages.filter(message => message.role === 'user').map(message => message.vi), currentOrderTarget);
      const timeoutMessage: ChatMessage = resolvedCount(timed.assessment) === 5 ? { role: 'clerk', vi: 'Hết giờ. Tôi đã chốt đơn rồi.', zh: '时间到了。我已经结束这张订单。' } : { role: 'clerk', vi: `Hết giờ rồi. Nhanh lên! ${next.vi}`, zh: `超时了，请快一点。${next.zh}` };
      const nextMessages = [...state.messages, timeoutMessage];
      setTimeouts(newTimeouts); setAssessment(timed.assessment); setMessages(nextMessages); setClerkMood('impatient'); setIsReacting(true);
      window.setTimeout(() => setIsReacting(false), 900);
      if (resolvedCount(timed.assessment) === 5) window.setTimeout(() => state.onComplete(timed.assessment, nextMessages, { timeouts: newTimeouts, responseTimes: state.responseTimes }), 850);
      else window.setTimeout(() => { setClerkMood('neutral'); setTimerEpoch(value => value + 1); }, 900);
    };
    const interval = window.setInterval(tick, 17);
    tick();
    return () => window.clearInterval(interval);
  }, [isSending, rush, rushStarted, timerEpoch, setAssessment, setMessages]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    const userMessage: ChatMessage = { role: 'user', vi: input.trim() }, pendingMessages = [...messages, userMessage], allUserInputs = pendingMessages.filter(message => message.role === 'user').map(message => message.vi), ruleAssessment = mergeAssessment(assessment, analyzeAttempts(userMessage.vi, currentOrderTarget)), resolved = resolvedCriteria(ruleAssessment), fallbackReply = clerkReply(resolved, allUserInputs, currentOrderTarget);
  ...
  return <div className={`cafe-scene dialogue-stage ${rush ? 'rush-mode' : ''}`}><div className="cafe-ambient" aria-hidden="true" /><aside className="cafe-task" aria-label="当前任务"><div className="cafe-task-title"><span>{rush ? '限时挑战 · DẬN' : '今日任务 · 01'}</span><strong>{orderSummary()}</strong><div className="progress-compact"><span>{progress}/5</span><div className="progress"><i style={{ width: `${progress * 20}%` }} /></div></div></div>{rush && <div className={`rush-timer ${urgent ? 'is-urgent' : ''}`} aria-label="本题剩余时间"><Timer size={15} /><span>{formatCountdown(remainingMs)}</span><small>超时 -5</small></div>}<div className="task-chips">{(Object.keys(assessment) as (keyof Assessment)[]).map(key => <span className={assessment[key] === 'correct' ? 'done' : assessment[key] === 'incorrect' ? 'incorrect' : ''} key={key} aria-label={`${criterionLabels[key]}：${assessment[key] === 'correct' ? '正确' : assessment[key] === 'incorrect' ? '错误' : '未作答'}`}>{assessment[key] === 'correct' && <Check size={13} />}{assessment[key] === 'incorrect' && <X size={13} />}{criterionLabels[key]}</span>)}</div></aside>
    ...
    {rush && !rushStarted && <div className="rush-arrival" role="dialog" aria-label="限时挑战说明"><div><span>NEW SHIFT · 新的店员</span><h3>店员似乎发生了变化。</h3><p>Dận 把订单本放在柜台上。他说话更直接，也没有太多耐心——每次回答都从 12 秒开始倒计时。</p><button onClick={() => { setRushStarted(true); setTimerEpoch(value => value + 1); }}>我准备好了 <ArrowRight size={17} /></button></div></div>}
  </div>;
}
...
function Report({ report, newlyUnlocked, onRetry, onMap }: { report: SavedReport; newlyUnlocked: boolean; onRetry: () => void; onMap: () => void }) {
  ...
  return <div className="report-page"><div className="report-heading"><div><p className="eyebrow">任务报告 · 已保存到本机</p><h2>{Object.values(report.assessment).every(value => value === 'correct') ? '任务完成，干得漂亮！' : '任务已结束，看看哪里还能进步。'}</h2><p className="lead">总分已包含任务正确性、AI 语言评价{report.difficulty === 'rush' ? '和限时表现' : ''}。</p></div><div className="score-card score-ready"><strong>{report.score}</strong><span>/ 100</span><p>最终综合得分</p></div></div><div className="report-grid"><section className="report-panel"><h3>任务正确性 · {report.objectiveScore}/75</h3>{(Object.keys(report.assessment) as (keyof Assessment)[]).map(key => <div className="report-row" key={key}><span>{criterionLabels[key]}</span><strong>{report.assessment[key] === 'correct' ? '✓ 15 / 15' : '× 0 / 15'}</strong></div>)}{report.hints > 0 && <p className="deduction">使用 {report.hints} 次提示，扣除 {report.hints * 2} 分。</p>}{report.difficulty === 'rush' && <div className="rush-report"><h3>限时表现</h3><div className="report-row"><span>平均反应时间</span><strong>{(average / 1000).toFixed(2)} 秒</strong></div><div className="report-row"><span>超时次数</span><strong>{report.timeouts} 次</strong></div>{report.timePenalty > 0 && <p className="deduction">超时扣除 {report.timePenalty} 分；超时题按首次作答错误锁定。</p>}</div>}{Object.values(report.assessment).includes('incorrect') && <p className="deduction">有项目首次作答错误，综合分最高为 79；语言分不能抵消任务错误。</p>}<h3>语言表达 · {report.feedback ? `${report.feedback.languageScore}/25` : '未取得'}</h3><p>{report.feedback ? 'AI 辅助评分不改变上方首次答案的对错。' : 'AI 评分本次未能连接，报告仅保留已经确定的任务分。'}</p></section><section className="report-panel"><h3>你的表达</h3>{report.expressions.map((text, index) => <p className="expression" key={index}>{text}</p>)}<h3>规范与建议</h3><p>{nonStandard.length > 0 ? '系统理解了你的无声调表达。正式书写时，请补全越南语声调符号。' : '你的表达包含了规范的越南语声调，继续保持。'}</p>{report.feedback && <><p><strong>语法：</strong>{report.feedback.grammar}</p><p><strong>词汇：</strong>{report.feedback.vocabulary}</p><p><strong>自然度：</strong>{report.feedback.naturalness}</p><h3>下一步练习</h3><ul>{report.feedback.advice.map((item, index) => <li key={index}>{item}</li>)}</ul></>}<div className="recommended"><small>推荐表达</small>{recommendedExpression}</div></section></div><div className="report-actions"><div className="map-return-wrap">{newlyUnlocked && <div className="unlock-bubble"><Sparkles size={16} /><span><strong>新难度已解锁</strong>返回街区，看看咖啡馆的新变化</span></div>}<button className="secondary-button" onClick={onMap}>返回城市地图</button></div><button className="primary-button" onClick={onRetry}><RotateCcw size={18} /> 再练一次</button></div></div>;
}
